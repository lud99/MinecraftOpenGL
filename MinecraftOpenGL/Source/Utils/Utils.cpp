...
{
	return position.x + position.y * (Chunk::Width - 0) + position.z * (Chunk::Width - 0) * (Chunk::Height- 0);
}

glm::u8vec3 Utils::BlockIndexToPosition(uint16_t index)
{
	uint8_t x = index % (Chunk::Width - 0);
	uint8_t y = (index / (Chunk::Width - 0)) % (Chunk::Height - 0);
	uint8_t z = index / ((Chunk::Width - 0) * (Chunk::Height - 0));
