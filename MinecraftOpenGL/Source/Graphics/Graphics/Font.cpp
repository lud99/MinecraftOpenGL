...
int Font::Load(const std::string& filepath)
{
    ...
    for (unsigned char c = 0; c < 128; c++)
    {
        ...
        {
            std::cout << "Freetype: Failed to load Glyph" << "\n";
            continue;
        }
        // generate texture
    ...
    m_Shader = ShaderLoader::CreateShader("Resources/Shaders/Text.vert", "Resources/Shaders/Text.frag");
    ...
    m_CharMesh.m_DynamicBufferSize = sizeof(TextureVertex) * 4 * m_MaxCountOfEachChar;
    m_CharMesh.CreateEmptyBuffer();

    m_Initialized = true;

    std::cout << "Loaded font: " << filepath.c_str() << "\n";
...
glm::vec2 Font::AddTextToBatch(const std::string& text, glm::vec2 position, float scale, glm::vec3 color, int align, float maxWidth)
{
    ...
    textSize.x = fabs(itPosition.x - startPosition.x);//glm::abs(itPosition - startPosition);
...
void Font::RenderBatch(glm::mat4 projectionMatrix, glm::mat4 viewMatrix)
{
    glm::vec2 textureCoords[4] = {
        glm::vec2(0.0f, 0.0f),
        glm::vec2(0.0f, 1.0f),
